...
import requests, string, time, re, sys, base64
...
RE_CT = re.compile(r"<b>Results:</b>\s*<pre>\s*([A-Za-z0-9+/=]+)\s*</pre>", re.I|re.S)

def get_ct(q, url=URL, timeout=TIMEOUT):
    r = requests.get(url, params={"query": q}, timeout=timeout)
    r.raise_for_status()
    m = RE_CT.search(r.text)
    if not m:
        raise RuntimeError("No ciphertext in response")
    b64str = m.group(1).strip()
    try:
        return base64.b64decode(b64str)
    except Exception as e:
        raise RuntimeError(f"Invalid base64 ciphertext: {b64str}") from e
def literal(s):
    return "'" + s.replace("'", "''") + "'"
