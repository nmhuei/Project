...
_sage_const_31337 = Integer(31337); _sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_8 = Integer(8); _sage_const_0 = Integer(0)
...
C = Matrix(GF(_sage_const_2 ), rows)
...
if gcd(E, ordC) != _sage_const_1 :
    ...
    bs = bytes(int(bitstring[i:i+_sage_const_8 ], _sage_const_2 ) for i in range(_sage_const_0 , len(bitstring), _sage_const_8 ))

    # 7) tìm flag trong bytes
    start = bs.find(b"crypto{")
    end   = bs.find(b"}", start)
    if start != -_sage_const_1  and end != -_sage_const_1 :
        flag = bs[start:end+_sage_const_1 ]
