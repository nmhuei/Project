...
import tarfile, os, gzip, pickle

# Base directory of this pw6 package, independent of current working directory
...
class IOSystem:
    def __init__(self):
        ...
        # On start: if students.dat exists, decompress and load data;
        # otherwise, fall back to interactive input.
        self.loadDataUsingPickle()
    
    # Tar + Gzip    
    def compress_all(self):
        """Write current data to text files and compress them into students.dat.

        Uses tar.gz as the compression format.
        """
        ...
        # Store compressed archive alongside this module
        archive_path = os.path.join(BASE_DIR, "students.dat")
        with tarfile.open(archive_path, "w:gz") as tar:
            for f in files:
                if os.path.exists(f):
                    # store only the base filename inside the archive
                    tar.add(f, arcname=os.path.basename(f))

        # Optionally remove plain text files after compressing
        for f in files:
            if os.path.exists(f):
                os.remove(f)

    def inputData(self):
        """On startup: load from students.dat if it exists, else ask for input.

        All files are stored in the same directory as this script, independent of cwd.
        """
        archive_path = os.path.join(BASE_DIR, "students.dat")
        if os.path.exists(archive_path):
            # Decompress all data files from the archive into the pw6 directory
            with tarfile.open(archive_path, "r:gz") as tar:
                tar.extractall(path=BASE_DIR)

            # Then load from the extracted text files in that directory
